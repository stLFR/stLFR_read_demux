...
if(@ARGV != 5)
{
        print "Example: perl split_barcode_SE_54RC_reads.1.pl barcode.list barcode_RC.list read_1.fq.gz 100 split_read \n";
        exit(0);
}

my $readlen = $ARGV[3];
my ($n1, $n2, $n3, $n4, $n5) = (10, 18, 10, 6, 10);
my %barcode_hash;
my %barcode_RC_hash;
...
while(<IN2>){
  ...
  if($n % 4 == 2){
    my $read = substr($line[0], 0, $readlen);
    my $b1 = substr($line[0], $readlen, $n1);
    my $b2 = substr($line[0], $readlen+$n1+$n2, $n3);
    my $b3 = substr($line[0], $readlen+$n1+$n2+$n3+$n4, $n5);
    if((exists $barcode_RC_hash{$b1}) && (exists $barcode_RC_hash{$b2}) && (exists $barcode_RC_hash{$b3})){
      my $hash = $barcode_RC_hash{$b1}."_".$barcode_RC_hash{$b2}."_".$barcode_RC_hash{$b3};
...
open OUT, "| gzip > $ARGV[4].1.fq.gz" or die "Can't write file";
